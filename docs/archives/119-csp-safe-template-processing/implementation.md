...
it('should handle predefined template variables', () => {
  const content =
    'Original: {{originalPrompt}}, Last: {{lastOptimizedPrompt}}, Input: {{iterateInput}}'
  ...
  const result = CSPSafeTemplateProcessor.processContent(content, context)
  expect(result).toBe(
    'Original: Write a story, Last: Write a creative story about space, Input: Make it more dramatic'
  )
})
```
