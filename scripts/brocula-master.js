...
class BroCulaMaster {
  ...
  async preFlightChecks() {
    ...
    const behindCount = execSync(`git rev-list --count HEAD..origin/${CONFIG.mainBranch}`, { encoding: 'utf8' }).trim();
  ...
  async monitorConsoleErrors() {
    ...
    try {
      ...
      if (fs.existsSync(reportPath)) {
        ...
        console.error(`   Found ${report.summary.totalErrors} errors, ${report.summary.totalWarnings} warnings`);
    ...
    try {
      ...
      if (fs.existsSync(reportPath)) {
        ...
        if (failingCategories.length > 0) {
          console.error(`❌ FATAL: Lighthouse scores below ${minScore} in: ${failingCategories.join(', ')}`);
  ...
  async applyFixes() {
    ...
    if (hasChanges) {
      ...
      try {
        execSync('git add -A');
        execSync('git diff --cached --quiet || git commit -m "fix(brocula): auto-fix console errors and lighthouse issues"');
  ...
  async managePullRequest() {
    ...
    try {
      const existingPR = execSync(`gh pr list --head ${branch} --json number --jq '.[0].number'`, { encoding: 'utf8' }).trim();
      ...
      if (existingPR) {
        ...
        execSync(`gh pr edit ${existingPR} --body "$(cat .github/pull_request_template.md 2>/dev/null || echo 'Auto-generated PR by BroCula')"`);
        ...
        execSync(`gh pr create --title "$(git log -1 --pretty=%s)" --body "${prBody}" --base ${CONFIG.mainBranch}`, {
          stdio: 'inherit'
        });
