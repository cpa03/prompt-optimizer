...
test.describe('变量管理器集成', () => {
  test('点击变量管理器按钮应该打开变量管理器', async ({ page }) => {
    ...
    // 注意：实际的选择器需要根据实际实现调整
    const variableManager = page.locator(
      '[data-testid="variable-manager"], .variable-manager, .n-modal'
    )
  ...
  test('变量管理器应该支持添加自定义变量', async ({ page }) => {
    ...
    if (await addButton.isVisible()) {
      ...
      const nameInput = page
        .locator('input[placeholder*="名称"], input[placeholder*="name"]')
        .first()
      const valueInput = page
        .locator(
          'input[placeholder*="值"], input[placeholder*="value"], textarea[placeholder*="值"]'
        )
        .first()
...
test.describe('测试面板模式化行为', () => {
  test('用户模式下测试面板应该显示变量提示', async ({ page }) => {
    ...
    // 如果有变量，应该显示变量提示
    // 注意：这需要先有包含变量的提示词内容
    const variableHint = page.locator(
      '[data-testid="variable-hint"], .variable-hint, text=/检测到变量|Variables Detected/'
    )

    // 检查是否存在（如果有变量的话）
    const isVisible = await variableHint
      .isVisible({ timeout: TIMEOUTS.SHORT * 10 })
      .catch(() => false)
  ...
  test('系统模式下应该显示测试输入区域', async ({ page }) => {
    ...
    const testInput = page.locator(
      '[data-testid="test-input"], textarea[placeholder*="测试"], textarea[placeholder*="问题"]'
    )
...
test.describe('变量值输入表单（完整实现）', () => {
  test('应该在有变量时显示变量值输入表单', async ({ page }) => {
    ...
    const formCard = page.locator(
      '.n-card:has-text("变量值设置"), .n-card:has-text("Variable Values")'
    )
  ...
  test('应该为每个变量提供输入框', async ({ page }) => {
    ...
    if (hasForm) {
      ...
      const variableInputs = page.locator(
        'input[placeholder*="变量值"], input[placeholder*="variable value"]'
      )
...
test.describe('双轮替换预览（完整实现）', () => {
  test('系统模式应该显示第一轮和第二轮替换', async ({ page }) => {
    ...
    const previewCard = page.locator(
      '.n-card:has-text("预览结果"), .n-card:has-text("Preview Result")'
    )

    const hasPreview = await previewCard
      .isVisible({ timeout: TIMEOUTS.PAGE_LOAD })
      .catch(() => false)

    if (hasPreview) {
      // 验证显示第一轮替换
      const firstRound = page.locator('text=/第一轮替换|First Round/')
      const hasFirstRound = await firstRound
        .isVisible({ timeout: TIMEOUTS.LONG })
        .catch(() => false)
  ...
  test('用户模式应该只显示最终预览', async ({ page }) => {
    ...
    if (hasPreview) {
      ...
      const hasFinalPreview = await finalPreview
        .isVisible({ timeout: TIMEOUTS.LONG })
        .catch(() => false)
