...
export function validateCustomModelConfig(config: CustomModelEnvConfig): ValidationResult {
  ...
  if (!config.suffix) {
    ...
    result.errors.push(
      `Invalid suffix: ${config.suffix}. Use 1-${MAX_SUFFIX_LENGTH} alphanumeric characters, underscores, or hyphens`
    )
    result.valid = false
  }
    ...
    try {
      ...
      if (!['http:', 'https:'].includes(url.protocol)) {
        result.warnings.push(
          `Unusual protocol in baseURL: ${url.protocol}. Expected http: or https:`
        )
      }
    } catch (error) {
...
export function waitForElectronApi(
  timeout: number = TIMEOUTS.service.electronInit
): Promise<boolean> {
 ...
 */
export function scanCustomModelEnvVars(
  useCache: boolean = true
): Record<string, ValidatedCustomModelEnvConfig> {
  // 如果启用缓存且有缓存结果，直接返回
