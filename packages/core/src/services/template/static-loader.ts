    ...
    try {
      ...
      Object.values(ALL_TEMPLATES).forEach((template) => {
        ...
        switch (templateType) {
          ...
          default:
            normalizedType =
              (templateType as any) === 'iterate'
                ? 'iterate'
                : (templateType as any) === 'optimize'
                  ? 'optimize'
                  : 'optimize'
            break
        }
      ...
      console.log(`✅ 成功加载 ${Object.keys(all).length} 个模板`, {
        ...
        'user-optimize':
          Object.keys(byType['user-optimize'].zh).length +
          Object.keys(byType['user-optimize'].en).length,
        text2imageOptimize:
          Object.keys(byType.text2imageOptimize.zh).length +
          Object.keys(byType.text2imageOptimize.en).length,
        image2imageOptimize:
          Object.keys(byType.image2imageOptimize.zh).length +
          Object.keys(byType.image2imageOptimize.en).length,
        imageIterate:
          Object.keys(byType.imageIterate.zh).length + Object.keys(byType.imageIterate.en).length,
        iterate: Object.keys(byType.iterate.zh).length + Object.keys(byType.iterate.en).length,
        'conversation-message-optimize':
          Object.keys(byType['conversation-message-optimize'].zh).length +
          Object.keys(byType['conversation-message-optimize'].en).length,
        'context-user-optimize':
          Object.keys(byType['context-user-optimize'].zh).length +
          Object.keys(byType['context-user-optimize'].en).length,
        'context-iterate':
          Object.keys(byType['context-iterate'].zh).length +
          Object.keys(byType['context-iterate'].en).length,
        evaluation:
          Object.keys(byType.evaluation.zh).length + Object.keys(byType.evaluation.en).length,
      })
