import { ITemplateManager, Template, TemplateType, templateSchema } from './types'
...
export class TemplateManager implements ITemplateManager {
  ...
  private builtinTemplatesLanguage: BuiltinTemplateLanguage | null = null
  ...
  private validateTemplateId(id: string | null | undefined): void {
    ...
    if (!TEMPLATE_CONFIG.idRegex.test(id)) {
      throw new TemplateValidationError(
        'Invalid template ID format: must be at least 3 characters, using only lowercase letters, numbers, and hyphens'
      )
    }
  }
  ...
  async listTemplates(): Promise<Template[]> {
    ...
    const templates = [...Object.values(builtinTemplates), ...userTemplates]
  ...
  private async getBuiltinTemplates(): Promise<Record<string, Template>> {
    ...
    if (this.builtinTemplatesCache && this.builtinTemplatesLanguage === currentLanguage) {
    ...
    this.builtinTemplatesLanguage = currentLanguage
  ...
  async listTemplatesByType(type: TemplateType): Promise<Template[]> {
    try {
      ...
      return templates.filter((template) => template.metadata.templateType === type)
    } catch (error) {
  ...
  async changeBuiltinTemplateLanguage(language: BuiltinTemplateLanguage): Promise<void> {
    if (this.builtinTemplatesLanguage !== language) {
    ...
    for (const template of templates) {
      try {
        ...
        if (builtinTemplate) {
          ...
          console.warn(
            `Detected conflict with built-in template ID: ${template.id}, renamed to: ${finalTemplateId}`
          )
        }
