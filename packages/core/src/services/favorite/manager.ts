...
export class FavoriteManager implements IFavoriteManager {
  ...
  async addFavorite(
    favorite: Omit<FavoritePrompt, 'id' | 'createdAt' | 'updatedAt' | 'useCount'>
  ): Promise<string> {
    await this.ensureInitialized()
    ...
    if (favorite.functionMode === 'basic' || favorite.functionMode === 'context') {
      if (!favorite.optimizationMode) {
        throw new FavoriteValidationError(
          `${favorite.functionMode} mode must specify optimizationMode`
        )
      }
    }
          ...
          favoritesToImport.forEach((favorite: any) => {
            try {
              ...
              const optimizationMode =
                favorite.optimizationMode || (functionMode !== 'image' ? 'system' : undefined)
              const imageSubMode =
                favorite.imageSubMode || (functionMode === 'image' ? 'text2image' : undefined)
              ...
              const useCount =
                typeof favorite.useCount === 'number' && favorite.useCount >= 0
                  ? favorite.useCount
                  : 0
              ...
              if (existingFavorite) {
                ...
                if (mergeStrategy === 'overwrite') {
                  ...
                  existingFavorite.description =
                    typeof favorite.description === 'string'
                  ...
                  existingFavorite.createdAt = parseTimestamp(
                    favorite.createdAt,
              ...
              const newFavorite: FavoritePrompt = {
                ...
                content: favorite.content,
                description:
                  typeof favorite.description === 'string' ? favorite.description : undefined,
                category,
                tags,
                functionMode,
                optimizationMode,
