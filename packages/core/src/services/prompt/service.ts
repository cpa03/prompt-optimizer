...
export class PromptService implements IPromptService {
  ...
  private checkDependencies() {
    ...
    if (!this.templateManager) {
      throw new ServiceDependencyError('TemplateManager', 'Template manager not initialized')
    }
    if (!this.historyManager) {
      throw new ServiceDependencyError('HistoryManager', 'History manager not initialized')
    }
  }
  ...
  private validateMessageOptimizationRequest(request: MessageOptimizationRequest) {
    ...
    // 验证选中的消息是否存在
    const selectedMessage = request.messages.find((msg) => msg.id === request.selectedMessageId)
    ...
    if (!selectedMessage.content?.trim()) {
      throw new OptimizationError('', 'Selected message content cannot be empty')
    }
  }
  ...
  async optimizePrompt(request: OptimizationRequest): Promise<string> {
    try {
      ...
      if (!template?.content) {
        throw new OptimizationError(request.targetPrompt, 'Template not found or invalid')
      }
  ...
  async optimizeMessage(request: MessageOptimizationRequest): Promise<string> {
    try {
      ...
      if (!template?.content) {
        throw new OptimizationError(selectedMessage.content, 'Template not found or invalid')
      }
      ...
      const selectedMessageData = {
        ...
        contentPreview:
          selectedMessage.content.length > maxLength
  ...
  async iteratePrompt(
    ...
    try {
      ...
      if (contextData?.messages && contextData.messages.length > 0) {
        const conversationText = TemplateProcessor.formatConversationAsText(contextData.messages)
        context.conversationContext = conversationText
      }

      // 如果有工具信息，将其格式化为文本并添加到上下文
      if (contextData?.tools && contextData.tools.length > 0) {
        const toolsText = TemplateProcessor.formatToolsAsText(contextData.tools)
        context.toolsContext = toolsText
      }
  ...
  private async getDefaultTemplateId(
    ...
    try {
      ...
      if (templateType === 'optimize' || templateType === 'conversationMessageOptimize') {
      ...
      for (const fallbackType of fallbackTypes) {
        const fallbackTemplates = await this.templateManager.listTemplatesByType(
          fallbackType as any
        )
  ...
  async testCustomConversationStream(
    ...
    try {
      ...
      if (request.tools && request.tools.length > 0) {
        ...
        // 传统的流式发送（无工具）
        await this.llmService.sendMessageStream(processedMessages, request.modelKey, {
          onToken: callbacks.onToken,
          onReasoningToken: callbacks.onReasoningToken,
          onComplete: async (response) => {
            if (response) {
