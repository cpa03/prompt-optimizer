...
describe.skipIf(!RUN_REAL_API)('Real API Integration Tests', () => {
  ...
  describe('OpenAI API 测试', () => {
    ...
    it.runIf(runOpenAITests)(
      '应该能使用OpenAI API优化提示词',
      async () => {
        // 执行优化
        const request = {
  ...
  describe('Custom API 测试', () => {
    ...
    it.runIf(runCustomTests)(
      '应该能使用Custom API优化提示词',
      async () => {
        // 执行优化
        const request = {
          optimizationMode: 'system' as const,
  ...
  describe('Gemini API 测试', () => {
    ...
    it.runIf(runGeminiTests)(
      '应该能使用Gemini API优化提示词',
      async () => {
        // 执行优化
        const request = {
          optimizationMode: 'system' as const,
  ...
  describe('DeepSeek API 测试', () => {
    ...
    it.runIf(runDeepSeekTests)(
      '应该能使用DeepSeek API优化提示词',
      async () => {
        // 执行优化
        const request = {
          optimizationMode: 'system' as const,
  ...
  describe('并发和错误处理测试', () => {
    ...
    it.runIf(runStabilityTests)(
      '应该能正确处理API错误',
      async () => {
        const models = await modelManager.getAllModels()
        const baseModel = models.find(
          (m) => m.enabled && m.providerMeta?.requiresApiKey && m.providerMeta?.id !== 'custom'
        )
        if (!baseModel) return
