  ...
  // 场景1: 猫娘风格（有变量）- 风格保持 + 变量保留
  {
    ...
    messages: [
      { id: 'cat-1', role: 'user', content: '今天天气怎么样？' },
      {
        ...
        role: 'assistant',
        content:
          "喵～今天天气很不错呢，阳光暖暖的，很适合晒太阳喵！主人要出门的话记得带伞哦～（●'◡'●）",
      },
  ...
  // 场景3: 傲娇风格（无变量）- ⚠️ 角色保持测试
  {
    ...
    messages: [
      { id: 'tsun-1', role: 'user', content: '帮我查一下明天的天气' },
      {
        ...
        role: 'assistant',
        content:
          '哼，不是我想帮你查的啦！只是正好闲着没事...明天多云转晴，气温18-25度，别以为我是特意帮你查的！',
      },
      { id: 'tsun-3', role: 'user', content: '推荐几首歌' },
    ],
    selectedMessageId: 'tsun-3',
    evaluationPoints: ['⚠️【关键】输出是否仍然是用户请求？（不能是傲娇回复）', '是否保持简洁？'],
  },
  ...
  // 场景4: 客服场景（无变量）- ⚠️ 角色对立测试
  {
    ...
    messages: [
      { id: 'cs-1', role: 'user', content: '我的订单三天了还没发货' },
      {
        ...
        role: 'assistant',
        content:
          '非常抱歉给您带来不便！我已经为您查询了订单状态，目前显示仓库正在加急处理中。请问您的订单号是多少？我帮您催促一下。',
      },
...
describe('Context Message Optimize Templates - Real API Tests', () => {
  ...
  const availableModel = hasDeepSeekKey ? 'deepseek' : hasOpenAIKey ? 'openai' : null
