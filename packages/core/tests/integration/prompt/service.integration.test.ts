...
describe('PromptService Integration Tests', () => {
  ...
  const delayBetweenTests = async () => {
    ...
    if (lastTestTime > 0 && timeSinceLastTest < DELAY_BETWEEN_TESTS) {
      ...
      console.log(
        `⏳ Waiting ${Math.round(waitTime / 1000)}s before next test to avoid rate limiting...`
      )
  ...
  beforeAll(() => {
    ...
    if (!hasGeminiKey) {
      console.log(
        'Skipping PromptService integration tests: GEMINI_API_KEY environment variable not set'
      )
    }
  })
  ...
  describe('optimizePrompt with different template formats', () => {
    it.runIf(hasGeminiKey)(
      'should work with string-based templates',
      async () => {
    ...
    it.runIf(hasGeminiKey)(
      'should work with message-based templates',
      async () => {
        await delayBetweenTests()
        // 添加一个消息模板 - 使用实际存在的变量
        ...
        const getTemplateSpy = vi
          ...
          .mockReturnValue(messageTemplate)
  ...
  describe('iteratePrompt with different template formats', () => {
    it.runIf(hasGeminiKey)(
      'should work with string-based iterate templates',
      async () => {
        await delayBetweenTests()
        // 添加一个简单的迭代模板供测试使用
        const simpleIterateTemplate: Template = {
          ...
          content: [
            ...
            },
            {
              role: 'user',
              content:
                'Improve this prompt: {{lastOptimizedPrompt}}\n\nSuggestion: {{iterateInput}}',
            },
          ] as MessageTemplate[],
          metadata: {
            version: '1.0',
        ...
        const getTemplateSpy = vi
          .spyOn(templateManager, 'getTemplate')
          .mockReturnValue(simpleIterateTemplate)
    ...
    it.runIf(hasGeminiKey)(
      'should work with message-based iterate templates',
      async () => {
        await delayBetweenTests()
        ...
        const getTemplateSpy = vi
          .spyOn(templateManager, 'getTemplate')
          .mockReturnValue(iterateTemplate)
  ...
  describe('streaming methods', () => {
    it.runIf(hasGeminiKey)(
      'should handle optimizePromptStream',
      async () => {
        await delayBetweenTests()
        const tokens: string[] = []
        let completed = false
    ...
    it.runIf(hasGeminiKey)(
      'should handle iteratePromptStream with template objects',
      async () => {
        await delayBetweenTests()
        const tokens: string[] = []
        let completed = false
        ...
        const getTemplateSpy = vi
          .spyOn(templateManager, 'getTemplate')
          .mockReturnValue(streamIterateTemplate)
  ...
  describe('error handling', () => {
    it.runIf(hasGeminiKey)('should handle template not found errors', async () => {
      ...
      await expect(promptService.optimizePrompt(request)).rejects.toThrow(/Template not found/)
    ...
    it.runIf(hasGeminiKey)('should handle invalid template content', async () => {
      ...
      const getTemplateSpy = vi
        .spyOn(templateManager, 'getTemplate')
        .mockReturnValue(invalidTemplate)
