...
describe.skipIf(!RUN_REAL_API)('Adapter Integration Tests - Real SDK', () => {
  ...
  describe('OpenAIAdapter Real API', () => {
    ...
    it.skipIf(!hasApiKey)(
      'should successfully call OpenAI API with sendMessage',
      async () => {
    ...
    it.skipIf(!hasApiKey)(
      'should successfully stream OpenAI API with callbacks',
      async () => {
        const apiKey = process.env.OPENAI_API_KEY || process.env.VITE_OPENAI_API_KEY
        const adapter = registry.getAdapter('openai')
        ...
        const messages: Message[] = [{ role: 'user', content: '请说"你好"' }]
    ...
    it.skipIf(!hasApiKey)(
      'should handle OpenAI API errors with stack trace',
      async () => {
        const adapter = registry.getAdapter('openai')
  ...
  describe('GeminiAdapter Real API', () => {
    ...
    it.skipIf(!hasApiKey)(
      'should successfully stream Gemini API',
      async () => {
        const apiKey = process.env.GEMINI_API_KEY || process.env.VITE_GEMINI_API_KEY
        const adapter = registry.getAdapter('gemini')
  ...
  describe('AnthropicAdapter Real API', () => {
    ...
    it.skipIf(!hasApiKey)(
      'should successfully stream Anthropic API',
      async () => {
        const apiKey = process.env.ANTHROPIC_API_KEY || process.env.VITE_ANTHROPIC_API_KEY
        const adapter = registry.getAdapter('anthropic')
  ...
  describe('Tool Calls Integration', () => {
    ...
    it.skipIf(!hasOpenAI)(
      ...
      async () => {
        ...
        const messages: Message[] = [{ role: 'user', content: '现在北京的天气怎么样?' }]
  ...
  describe('ModelScopeAdapter Real API', () => {
    ...
    it.skipIf(!hasApiKey)(
      'should successfully call ModelScope API with sendMessage',
      async () => {
    ...
    it.skipIf(!hasApiKey)(
      'should successfully stream ModelScope API with callbacks',
      async () => {
        const apiKey = process.env.MODELSCOPE_API_KEY || process.env.VITE_MODELSCOPE_API_KEY
        const adapter = registry.getAdapter('modelscope')
  ...
  describe('Error Handling', () => {
    it('should throw clear error for unknown provider', () => {
      expect(() => registry.getAdapter('unknown-provider')).toThrow(
        /Unknown (provider|文本模型提供商): unknown-provider/
      )
    })
