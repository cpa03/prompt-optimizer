import { createLLMService, ModelManager, RequestConfigError } from '../../../src/index.js'
...
describe('LLM 服务通用测试', () => {
  ...
  describe('API 调用错误处理', () => {
    it('应该能正确处理无效的消息格式', async () => {
      ...
      await expect(async () => {
        await llmService.sendMessage([{ role: 'invalid', content: '测试消息' }], testModel)
    ...
    it('应该能正确处理未启用的模型', async () => {
      ...
      const messages = [{ role: 'user', content: '你好，我们来玩个游戏' }]
