...
describe.skipIf(!RUN_REAL_API)('OpenAI API 真实连接测试', () => {
  ...
  const availableKeys = openaiCompatibleKeys.filter(
  ...
  console.log(
    `使用 ${modelConfig.key} 进行 OpenAI 兼容 API 测试，模型: ${modelConfig.defaultModel}`
  )
  ...
  it('应该能正确使用高级参数', async () => {
    ...
    try {
      ...
      const messages = [{ role: 'user', content: '请用一句话回答：什么是人工智能？' }]
