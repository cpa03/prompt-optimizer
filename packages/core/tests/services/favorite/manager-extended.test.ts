...
describe('FavoriteManager - 扩展功能', () => {
  ...
  beforeEach(() => {
    ...
    storageProvider = {
      ...
      batchUpdate: vi.fn(
        async (operations: Array<{ key: string; operation: 'set' | 'remove'; value?: string }>) => {
  ...
  describe('ensureDefaultCategories - 默认分类管理', () => {
    ...
    it('用户删除所有分类后不应自动重建', async () => {
      // 首次创建
      await manager.ensureDefaultCategories([{ name: 'Cat1', color: '#FF0000' }])
    ...
    it('已有分类时不应重复创建', async () => {
      ...
      await manager.ensureDefaultCategories([{ name: 'New Category', color: '#FFFFFF' }])
