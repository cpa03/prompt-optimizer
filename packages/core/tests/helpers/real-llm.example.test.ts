    ...
    it('应该能检测到可用的提供商', () => {
      ...
      if (available.length > 0) {
        available.forEach((provider, index) => {
          ...
          console.log(
            `   - 模型: ${provider.modelConfig.modelMeta.name} (${provider.modelConfig.modelMeta.id})`
          )
          console.log(
            `   - Base URL: ${provider.modelConfig.connectionConfig.baseURL || 'default'}`
          )
        })
      }
  ...
  describe('多轮对话示例', () => {
    it.skipIf(!hasAvailableProvider())(
      ...
      async () => {
        ...
        // 第一轮
        const messages1: Message[] = [{ role: 'user', content: '我的名字叫Alice' }]
  ...
  describe('性能和稳定性示例', () => {
    it.skipIf(!hasAvailableProvider())(
      ...
      async () => {
        ...
        const messages: Message[] = [{ role: 'user', content: '说"你好"' }]
