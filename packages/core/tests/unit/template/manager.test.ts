...
describe('TemplateManager with Mocked LanguageService', () => {
  ...
  it('should not allow overwriting a builtin template', async () => {
    ...
    await expect(templateManager.saveTemplate(maliciousTemplate)).rejects.toThrow(
      'Cannot overwrite built-in template: test-en'
    )
  })
  ...
  it('should not allow deleting a built-in template', async () => {
    await expect(templateManager.deleteTemplate('test-en')).rejects.toThrow(
      'Cannot delete built-in template: test-en'
    )
  })
})
