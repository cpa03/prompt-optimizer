...
describe.skipIf(!RUN_REAL_API)('LLM Parameters (llmParams) Functionality', () => {
  ...
  const hasOpenAICompatibleKey = openaiCompatibleKeys.some(
    (key) => process.env[key] && process.env[key].trim()
  )

  const hasGeminiKey = geminiKeys.some((key) => process.env[key] && process.env[key].trim())
  ...
  describe('OpenAI Compatible Providers', () => {
    ...
    it('should use custom parameters from llmParams', async () => {
      ...
      const messages = [{ role: 'user' as const, content: 'Say exactly: "Hello World"' }]
  ...
  describe('Gemini Provider', () => {
    ...
    it('should use Gemini-specific parameters from llmParams', async () => {
      ...
      const messages = [{ role: 'user' as const, content: 'Tell me a very short fact about AI' }]
