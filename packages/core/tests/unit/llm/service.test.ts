...
import { LLMService, ModelManager, APIError, RequestConfigError, Message } from '../../../src/index'
...
describe('LLMService', () => {
  ...
  describe('validateModelConfig', () => {
    it('should throw error when model is disabled', () => {
      ...
      expect(() => service['validateModelConfig'](disabledConfig)).toThrow()
    })
    ...
    it('should allow empty apiKey for services like Ollama', () => {
      ...
      expect(() => service['validateModelConfig'](configWithEmptyApiKey)).not.toThrow()
    })
    ...
    it('should throw error when provider is missing', () => {
      ...
      expect(() => service['validateModelConfig'](invalidConfig)).toThrow()
    })
  ...
  describe('validateMessages', () => {
    ...
    it('should throw error for empty messages', () => {
      expect(() => service['validateMessages']([])).toThrow()
    })

    it('should throw error for invalid role', () => {
      const invalidMessages: Message[] = [{ role: 'invalid' as any, content: 'test' }]
      expect(() => service['validateMessages'](invalidMessages)).toThrow()
    })
  ...
  describe('testConnection', () => {
    it('should allow connection test when model is disabled', async () => {
      ...
      const sendSpy = vi.spyOn(adapter, 'sendMessage').mockResolvedValue({ content: 'ok' })
  ...
  describe('fetchModelList', () => {
    ...
    it('should return merged dynamic + static models when dynamic fetch succeeds', async () => {
      ...
      vi.spyOn(adapter, 'getModelsAsync').mockResolvedValue([dynamicModel])
