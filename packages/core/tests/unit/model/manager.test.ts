...
describe('ModelManager', () => {
  ...
  describe('addModel', () => {
    ...
    it('should throw ModelConfigError when adding a model with an existing key', async () => {
      ...
      await expect(
        modelManager.addModel('existingKey', createTextModelConfig('existingKey', 'DuplicateKey'))
    ...
    it('should throw ModelConfigError when adding a model with invalid config', async () => {
      ...
      await expect(
        modelManager.addModel('invalidKey', invalidModel as TextModelConfig)
      ).rejects.toThrow(ModelConfigError)
    })
  })
  ...
  describe('updateModel', () => {
    ...
    it('should throw ModelConfigError when updating a non-existent model', async () => {
      await expect(modelManager.updateModel('nonExistentKey', { name: 'NewName' })).rejects.toThrow(
        ModelConfigError
      )
    })
  })
  ...
  describe('paramOverrides deep copy', () => {
    it('should deep copy paramOverrides to avoid reference sharing when adding models', async () => {
      ...
      const modelConfig = createTextModelConfig(
        'TestModel',
        'TestModel',
        true,
        'test_key',
  ...
  describe('paramOverrides security validation', () => {
    it('should reject dangerous parameters when adding models', async () => {
      const modelWithDangerousParams = createTextModelConfig(
        'DangerousModel',
        'DangerousModel',
        true,
        'test_key',
      ...
      modelWithDangerousParams.paramOverrides = {
        ...
        constructor: function () {
          return 'hack'
        }, // Another dangerous parameter
      }

      await expect(
        modelManager.addModel('dangerous-model', modelWithDangerousParams)
      ).rejects.toThrow(ModelConfigError)
    })

    it('should reject invalid parameter types when adding models', async () => {
      const modelWithInvalidTypes = createTextModelConfig(
        'InvalidModel',
        'InvalidModel',
        true,
        'test_key',
      ...
      await expect(modelManager.addModel('invalid-model', modelWithInvalidTypes)).rejects.toThrow(
        ModelConfigError
      )
    })

    it('should reject out-of-range parameters when adding models', async () => {
      const modelWithOutOfRangeParams = createTextModelConfig(
        'OutOfRangeModel',
        'OutOfRangeModel',
        true,
        'test_key',
      ...
      await expect(
        modelManager.addModel('out-of-range-model', modelWithOutOfRangeParams)
      ).rejects.toThrow(ModelConfigError)
    })

    it('should accept valid parameters when adding models', async () => {
      const modelWithValidParams = createTextModelConfig(
        'ValidModel',
        'ValidModel',
        true,
        'test_key',
      ...
      await expect(
        modelManager.addModel('valid-model', modelWithValidParams)
    ...
    it('should validate paramOverrides when updating models', async () => {
      const initialModel = createTextModelConfig(
        'TestModel',
        'TestModel',
        true,
        'test_key',
      ...
      await expect(
        modelManager.updateModel('test-model', {
          paramOverrides: {
            temperature: 0.5,
    ...
    it('should validate provider-specific parameters', async () => {
      ...
      await expect(modelManager.addModel('gemini-model', geminiModel)).resolves.not.toThrow()
