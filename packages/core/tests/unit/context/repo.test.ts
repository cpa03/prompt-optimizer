...
describe('ContextRepo', () => {
  ...
  describe('基础查询操作', () => {
    ...
    it('get() 对不存在的ID应该抛出NOT_FOUND错误', async () => {
      await expect(repo.get('non-existent-id')).rejects.toThrow(ContextError)
    ...
    it('setCurrentId() 对不存在的ID应该抛出NOT_FOUND错误', async () => {
      await expect(repo.setCurrentId('non-existent-id')).rejects.toThrow(ContextError)
  ...
  describe('上下文创建', () => {
    ...
    it('duplicate() 对不存在的ID应该抛出NOT_FOUND错误', async () => {
      await expect(repo.duplicate('non-existent-id')).rejects.toThrow(ContextError)
  ...
  describe('上下文删除', () => {
    ...
    it('remove() 应该删除指定上下文', async () => {
      ...
      await expect(repo.get(newId)).rejects.toThrow(ContextError)
    })
    ...
    it('remove() 应该拒绝删除最后一个上下文', async () => {
      // 只有默认上下文存在时尝试删除
      await expect(repo.remove(DEFAULT_CONTEXT_CONFIG.id)).rejects.toThrow(ContextError)
    ...
    it('remove() 对不存在的ID应该抛出NOT_FOUND错误', async () => {
      await expect(repo.remove('non-existent-id')).rejects.toThrow(ContextError)
  ...
  describe('错误处理', () => {
    ...
    it('应该在存储操作失败时抛出STORAGE_ERROR', async () => {
      ...
      await expect(repo.create({ title: 'test' })).rejects.toThrow('Storage failed')
    })
    ...
    it('应该在数据解析失败时抛出STORAGE_ERROR', async () => {
      ...
      await expect(newRepo.list()).rejects.toThrow(ContextError)
    })

    it('应该验证无效的上下文ID格式', async () => {
      await expect(repo.get('')).rejects.toThrow(ContextError)
  ...
  describe('并发安全性', () => {
    ...
    it('应该处理并发创建操作', async () => {
      const promises = Array.from({ length: 3 }, (_, i) => repo.create({ title: `并发上下文${i}` }))
