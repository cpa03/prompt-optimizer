  ...
  private async setupDefaultModel(config: MCPServerConfig): Promise<void> {
    ...
    try {
      ...
      if (mcpModel) {
        ...
        logger.info(`   Base URL: ${mcpModel.connectionConfig.baseURL || 'default'}`)
      } else {
        logger.info(
          `Default model configured with preferred provider: ${config.preferredModelProvider || 'auto-selected'}`
        )
      }
    } catch (error) {
