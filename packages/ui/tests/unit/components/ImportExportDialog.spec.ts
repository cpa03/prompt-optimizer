...
const naiveStubs = {
  ...
  NInput: {
    name: 'NInput',
    template:
      '<textarea v-if="type === \'textarea\'" :value="value" @input="$emit(\'update:value\', $event.target.value)"></textarea>',
...
describe('ImportExportDialog', () => {
  ...
  it('应该提供完整的导入格式选项', () => {
    ...
    expect(wrapper.vm.importFormats.map((f) => f.id)).toEqual([
      'smart',
      'openai',
      'langfuse',
      'conversation',
  ...
  it('导出预览应该与实际导出结构一致', async () => {
    const wrapper = createWrapper({
      ...
      tools: [
        { type: 'function', function: { name: 'tool', description: 'desc', parameters: {} } },
      ],
    })
