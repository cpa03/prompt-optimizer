  ...
  test: {
    ...
    error: {
      ...
      forbiddenTemplateSyntax:
        '不支援使用未轉義 Mustache 語法（如三花括號或 & 標籤），請使用一般變數占位符',
  ...
  toast: {
    ...
    warning: {
      ...
      chunkLoadRefreshConfirm:
        '偵測到應用已更新。為繼續使用，請重新整理頁面（不會清除歷史紀錄和設定）。現在重新整理嗎？',
  ...
  evaluation: {
    ...
    variableExtraction: {
      ...
      invalidVariableNames:
        '以下變數名稱不合法（不能以數字或 # / ^ ! > & 開頭，且不能包含空白/花括號，最長 {max} 個字元）：{names}',
    },
  ...
  error: {
    ...
    image: {
      ...
      config_provider_mismatch:
        '設定提供商不匹配: config={configProviderId}, adapter={adapterProviderId}',
      connection_config_missing_field: '連線設定缺少必填欄位: {field}',
      connection_config_invalid_field_type:
        '連線設定欄位型別錯誤: {field} 需要 {expectedType}，實際 {actualType}',
