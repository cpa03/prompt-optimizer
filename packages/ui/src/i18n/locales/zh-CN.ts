  ...
  test: {
    ...
    error: {
      ...
      forbiddenTemplateSyntax:
        '不支持使用未转义 Mustache 语法（如三花括号或 & 标签），请使用普通变量占位符',
  ...
  toast: {
    ...
    warning: {
      ...
      chunkLoadRefreshConfirm:
        '检测到应用已更新。为继续使用，请刷新页面（不会清除历史记录和设置）。现在刷新吗？',
  ...
  evaluation: {
    ...
    variableExtraction: {
      ...
      invalidVariableNames:
        '以下变量名不合法（不能以数字或 # / ^ ! > & 开头，且不能包含空白/花括号，最长 {max} 个字符）：{names}',
    },
  ...
  error: {
    ...
    image: {
      ...
      config_provider_mismatch:
        '配置提供商不匹配: config={configProviderId}, adapter={adapterProviderId}',
      connection_config_missing_field: '连接配置缺少必填字段: {field}',
      connection_config_invalid_field_type:
        '连接配置字段类型错误: {field} 需要 {expectedType}，实际 {actualType}',
