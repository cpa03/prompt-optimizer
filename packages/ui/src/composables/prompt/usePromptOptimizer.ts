...
export function usePromptOptimizer(
      ...
      await promptService.value!.optimizePromptStream(request, {
        ...
        onComplete: async () => {
          ...
          try {
            ...
            const baseType = (
              optimizationMode.value === 'system' ? 'optimize' : 'userOptimize'
            ) as PromptRecordType
            const recordType = (() => {
              if (isPro) {
                return (
              ...
              if (tplType === 'conversationMessageOptimize' || tplType === 'contextUserOptimize')
                return tplType as PromptRecordType
              return baseType
            })()
    ...
    try {
      ...
      const targetPrompt =
        ...
        (advancedContext.messages && Object.keys(advancedContext.messages).length > 0
          ? t('toast.info.multiTurnOptimizationPrompt', {
              count: Object.keys(advancedContext.messages).length,
            })
          : '')
            ...
            const recordData = {
              ...
              metadata: {
                ...
                conversationSnapshot: advancedContext.messages?.map((msg) => ({
                  ...
                  chainId: (msg as unknown as Record<string, unknown>).chainId as
                    | string
                    | undefined,
                  appliedVersion: (msg as unknown as Record<string, unknown>).appliedVersion as
                    | number
                    | undefined,
                })),
              },
  ...
  // 迭代优化
  state.handleIteratePrompt = async ({
    originalPrompt,
  ...
  state.saveLocalEdit = async ({
    ...
    try {
      ...
      const currentRecord = state.currentVersions.find(
        (v: { id: string; modelKey?: string; templateId?: string }) =>
      ...
      const templateId =
        ...
        state.selectedIterateTemplate?.id ||
        (optimizationMode.value === 'system'
          ? state.selectedOptimizeTemplate?.id
          : state.selectedUserOptimizeTemplate?.id) ||
        'local-edit'
