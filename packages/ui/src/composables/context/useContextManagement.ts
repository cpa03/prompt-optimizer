...
export function useContextManagement(options: ContextManagementOptions) {
  ...
  const initializeContextPersistence = async () => {
    ...
    try {
      ...
      console.warn('[useContextManagement] Failed to initialize context persistence:', error)
    } finally {
  ...
  const handleOpenContextEditor = async (
    ...
    try {
      ...
      console.warn('[useContextManagement] Variable manager refresh failed:', e)
    }
  ...
  const handleContextModeChange = async (mode: ContextMode) => {
    ...
    try {
      ...
      console.error('[useContextManagement] Failed to change context mode:', error)
