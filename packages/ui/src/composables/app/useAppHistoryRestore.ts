...
export function useAppHistoryRestore(options: AppHistoryRestoreOptions): AppHistoryRestoreReturn {
  ...
  const handleHistoryReuseImpl = async (context: HistoryContext) => {
    ...
    if (
      ...
      // 根据记录类型设置正确的图像子模式
      const meta =
        (isRecord(record.metadata) ? record.metadata : null) ??
       ...
       */
      if (rt === 'contextUserOptimize' || (targetFunctionMode === 'pro' && targetMode === 'user')) {
        await nextTick()
      ...
      if (rt === 'conversationMessageOptimize' || rt === 'contextSystemOptimize') {
        ...
        let conversationSnapshot: ConversationSnapshotMessage[] | undefined
        ...
        const targetMessage = messageId
          ? (proMultiMessageSession.conversationMessagesSnapshot || []).find(
