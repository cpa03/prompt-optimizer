    ...
    >
      ...
      >
        ...
        <n-form-item :label="t('favorites.categoryManager.categoryColor')" path="color">
          <n-color-picker v-model:value="categoryForm.color" :modes="['hex']" :show-alpha="false" />
        </n-form-item>
      </n-form>
      ...
      <p v-if="deletingCategoryHasChildren" style="color: var(--n-color-error)">
        {{
          t('favorites.categoryManager.deleteChildrenWarning', {
            count: deletingCategoryChildCount,
          })
        }}
      </p>
...
const formRules = computed<FormRules>(() => ({
  name: [
    {
      required: true,
      message: t('favorites.categoryManager.validation.nameRequired'),
      trigger: ['input', 'blur'],
    },
    {
      min: 1,
      max: 50,
      message: t('favorites.categoryManager.validation.nameLength'),
      trigger: ['input', 'blur'],
    },
  ],
}))
...
const renderSuffix = ({ option }: { option: TreeOption }): VNodeChild => {
  ...
  return h('div', { class: 'tree-suffix', onClick: (e: Event) => e.stopPropagation() }, [
    h(
      NDropdown,
      {
...
const handleDeleteCategory = async (category: FavoriteCategory) => {
  ...
  if (servicesValue?.favoriteManager) {
    try {
      deletingCategoryUsageCount.value = await servicesValue.favoriteManager.getCategoryUsage(
        category.id
      )
    } catch (_error) {
