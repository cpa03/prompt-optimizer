  ...
  <ToastUI>
    ...
    >
        ...
        <template v-if="filteredFavorites.length === 0">
          ...
          <div class="favorites-empty-state">
            <n-empty
              :description="
                searchKeyword
                  ? t('favorites.manager.emptySearchResult')
                  : t('favorites.manager.emptyDescription')
              "
              size="large"
            >
              <template #icon>
      ...
      >
        <n-form label-placement="top">
          ...
          <n-form-item :label="t('favorites.manager.importDialog.mergeStrategy')">
            <n-radio-group v-model:value="importState.mergeStrategy">
              <n-radio-button value="skip">{{
                t('favorites.manager.importDialog.skipDuplicate')
              }}</n-radio-button>
              <n-radio-button value="overwrite">{{
                t('favorites.manager.importDialog.overwriteDuplicate')
              }}</n-radio-button>
              <n-radio-button value="merge">{{
                t('favorites.manager.importDialog.createCopy')
              }}</n-radio-button>
            </n-radio-group>
          </n-form-item>
        </n-form>
        <template #action>
          <n-space justify="end">
            <n-button @click="closeImportDialog" :disabled="importState.importing">{{
...
import { Search, DotsVertical, Upload, Download, Trash, Plus, Tags, Folder } from '@vicons/tabler'
...
const actionMenuOptions = computed(() => [
  {
    label: () =>
      h(
        'span',
        { 'data-testid': 'favorites-manager-action-manage-tags' },
        t('favorites.manager.actions.manageTags')
      ),
    key: 'manageTags',
  ...
  },
  {
    label: () =>
      h(
        'span',
        { 'data-testid': 'favorites-manager-action-export' },
        t('favorites.manager.actions.export')
      ),
    key: 'export',
  ...
  },
  {
    label: () =>
      h(
        'span',
        { 'data-testid': 'favorites-manager-action-clear' },
        t('favorites.manager.actions.clear')
      ),
    key: 'clear',
...
const handleImportConfirm = async () => {
  ...
  try {
    ...
    message.success(
      t('favorites.manager.importDialog.importSuccess', {
    ...
    if (result.errors.length > 0) {
      message.warning(
        `${t('favorites.manager.importDialog.importPartialFailed')}：\n${result.errors.join('\n')}`
      )
    }
    await loadFavorites()
    closeImportDialog()
...
const handleCopyFavorite = async (favorite: FavoritePrompt) => {
  const copied = await tryCopyToClipboard(
    favorite.content,
    t('favorites.manager.actions.copySuccess')
  )
  if (!copied) return
...
const previewDialogTitle = computed(() => {
  ...
  const categoryName = previewFavorite.value.category
    ...
    : ''
  const updatedLabel = t('favorites.manager.preview.updatedAt', {
    time: formatDate(previewFavorite.value.updatedAt),
  })
