    ...
    >
      ...
      <div class="split-pane" style="min-width: 0; height: 100%; overflow: hidden">
        <NFlex vertical :style="{ overflow: 'auto', height: '100%', minHeight: 0 }" :size="12">
          ...
          <NCard :style="{ flexShrink: 0, overflow: 'auto' }" content-style="padding: 0;">
            <ConversationManager
          ...
          <NCard :style="{ flexShrink: 0 }" size="small">
            <NFlex vertical :size="12">
              ...
              >
                {{
                  displayAdapter.displayedIsOptimizing.value
                    ? $t('common.loading')
...
const globalVariables = computed(
  () => variableManager?.variableManager.value?.listVariables() || {}
)

const predefinedVariables = computed(() => {
  // 从 PREDEFINED_VARIABLES 常量获取预定义变量
  return PREDEFINED_VARIABLES.reduce(
    (acc, name) => {
...
const handleOpenPromptPreview = () => {
  openPromptPreview(displayAdapter.displayedOptimizedPrompt.value || '', {
...
const displayAdapter = usePromptDisplayAdapter(conversationOptimization, {
...
const optimizationContextToolsRef = inject<Ref<ToolDefinition[]>>(
...
const selectedTemplateIdModel = computed({
  ...
  set: (value) => {
    templateSelection.selectedTemplateId.value = value
  },
})
...
onMounted(() => {
  ...
  if (proMultiSession.selectedMessageId) {
    const restored = (conversationMessages.value || []).find(
      (m) => m.id === proMultiSession.selectedMessageId
    )
    if (restored) {
...
const isContextSystemHistoryPayload = (value: unknown): value is ContextSystemHistoryPayload => {
  if (!isObjectRecord(value)) return false
  ...
  if (conversationSnapshot !== undefined && !Array.isArray(conversationSnapshot)) return false
...
const restoreFromHistory = async (payload: unknown) => {
  if (!isContextSystemHistoryPayload(payload)) {
    console.warn('[ContextSystemWorkspace] Invalid history payload, ignored:', payload)
    return
  }
...
@media (max-width: 768px) {
  ...
  .context-system-split > *:nth-child(2) {
    display: none;
  }
