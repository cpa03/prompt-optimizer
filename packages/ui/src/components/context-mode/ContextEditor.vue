<template>
  ...
  >
      ...
      <NSpace vertical :size="12" style="padding-right: 12px">
        ...
        >
          <template #header>
            <NSpace justify="space-between" align="center">
              <NSpace align="center" :size="4">
                ...
                >
                  {{
                    t('contextEditor.variableDetected', {
                      count: getMessageVariables(message.content).detected.length,
                    })
                  }}
                </NTag>
                <NTag
                ...
                >
                  {{
                    t('contextEditor.missingVariableLabel', {
                      count: getMessageVariables(message.content).missing.length,
                    })
                  }}
                </NTag>
              </NSpace>
          ...
          <VariableAwareInput
            ...
            :existing-temporary-variables="
              Object.keys(aggregatedVars.variablesBySource.value.temporary)
            "
        ...
        <!-- 添加消息按钮 -->
        <NButton @click="addMessage" :size="buttonSize" dashed type="primary" :disabled="disabled">
          <template #icon>
    ...
    <template #action>
      <NSpace justify="space-between">
        <NSpace>
          ...
          <NButton
            ...
            :disabled="
              disabled ||
              loading ||
              (localState.messages.length === 0 && localState.tools.length === 0)
            "
          >
            <template #icon>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor">
        ...
        <NSpace>
          <NButton @click="handleCancel" :size="buttonSize" :disabled="loading">
            {{ t('common.cancel') }}
          </NButton>
          <NButton @click="handleSave" :size="buttonSize" type="primary" :loading="loading">
            {{ t('common.save') }}
          </NButton>
        </NSpace>
      </NSpace>
  ...
  <NModal
    ...
    preset="card"
    :title="
      variableEditState.isEditing ? t('contextEditor.editVariable') : t('contextEditor.addVariable')
    "
  ...
  >
    <NSpace vertical>
      ...
      <div>
        ...
        />
        <NText depth="3" class="text-xs mt-1" v-if="isPredefinedVariable(variableEditState.name)">
      ...
      <!-- 变量类型 -->
      <div>
        <label class="block text-sm font-medium mb-2">{{ t('contextEditor.variableType') }}</label>
    ...
    <template #action>
      <NSpace justify="end">
        ...
        <NButton
          ...
          type="primary"
          :size="buttonSize"
          :disabled="!variableEditState.name.trim() || isPredefinedVariable(variableEditState.name)"
        >
  ...
  <!-- 实时区域用于屏幕阅读器 -->
  <div role="status" aria-live="polite" aria-atomic="true" class="sr-only" v-if="liveRegionMessage">
    {{ liveRegionMessage }}
  </div>
...
import type { ContextEditorProps } from '../../types/components'
import type {
  ContextEditorState,
  ConversationMessage,
  ToolDefinition,
...
if (!props.variableManager) {
  throw new Error(
    '[ContextEditor] Missing required prop: variableManager. ContextEditor must receive a variableManager instance from parent component.'
  )
}
...
// 响应式配置
const { modalWidth, buttonSize: responsiveButtonSize, isMobile } = useResponsive()
...
// 通用标签页可见性计算函数
const createTabVisibility = (tabName: TabName) =>
  computed(() => {
...
const resolveDefaultTab = (): string => {
  ...
  const preferenceOrder: Array<keyof typeof visibilityMap> = ['messages', 'variables', 'tools']
...
const saveVariable = () => {
  ...
  const typeLabel =
    type === 'temporary'
      ? t('contextEditor.variableSourceLabels.temporary')
      : t('contextEditor.variableSourceLabels.global')
...
const setMessageRef = (index: number, refEl: Element | ComponentPublicInstance | null) => {
...
watch([showMessagesTab, showVariablesTab, showToolsTab], () => {
  const visibilityMap: Record<string, boolean> = {
    messages: showMessagesTab.value,
    variables: showVariablesTab.value,
    tools: showToolsTab.value,
