          ...
          <NCard :style="{ flexShrink: 0 }">
            ...
            <NFlex v-if="isInputPanelCollapsed" justify="space-between" align="center">
              <NFlex align="center" :size="8">
                ...
                <NText v-if="originalPrompt" depth="3" style="font-size: 12px">
                  {{ promptSummary }}
                </NText>
              </NFlex>
              <NButton
                  ...
                  <NSpace vertical :size="8">
                    ...
                    <template v-if="services && services.templateManager">
                      <SelectWithConfig
                        ...
                        @config="() => onOpenTemplateManager(templateType)"
                      />
                    </template>
                ...
                <NGridItem :span="6" :xs="24" :sm="6" class="flex items-end justify-end">
                  <NSpace :size="8">
                    ...
                    <NButton
                      ...
                      :loading="isAnalyzing"
                      @click="handleAnalyze"
                      :disabled="isAnalyzing || isOptimizing || !originalPrompt.trim()"
                    >
                      {{
                    ...
                    <template v-if="hasVariantResult(id)">
                      <NSpace vertical :size="12" style="padding: 12px">
                        ...
                        <template v-if="getVariantResult(id)?.text">
                          <NCard size="small" :title="t('imageWorkspace.results.textOutput')">
                        ...
                        <NSpace justify="center" :size="8">
                          ...
                          <NButton
                            ...
                            :aria-label="
                              isCopySuccess(id)
                                ? t('imageWorkspace.results.copied')
                          ...
                          >
                            ...
                            <Transition name="text-fade" mode="out-in">
                              <span v-if="isCopySuccess(id)" key="success">{{
                                t('imageWorkspace.results.copied')
                              }}</span>
...
} from '../../composables/prompt/useEvaluationHandler'
...
// 图像生成相关
const { imageModels, generateText2Image, validateText2ImageRequest, loadImageModels } =
  useImageGeneration()
...
const getVariantRequest = (id: TestVariantId): Text2ImageRequest | null => {
  ...
  if (ctx.missingVariables.length > 0) {
    toast.error(
      t('imageWorkspace.generation.missingVariables', { vars: ctx.missingVariables.join(', ') })
    )
    return null
  }
...
const appOpenTemplateManager = inject<((type?: TemplateEntryType) => void) | null>(
  'openTemplateManager',
  null
)
...
const handleSaveFavorite = (data: { content: string; originalContent?: string }) => {
  ...
  if (appHandleSaveFavorite) {
    ...
    console.warn('[ImageText2ImageWorkspace] handleSaveFavorite not available from App.vue')
  }
}
...
const handleRestoreHistory = async (event: Event) => {
  ...
  const record =
    (requestedVersionId && versions.find((v) => v.id === requestedVersionId)) ||
...
const refreshTextModelsHandler = async () => {
  try {
    ...
    console.warn('[ImageText2ImageWorkspace] Failed to refresh text models after manager close:', e)
  }
}
