  ...
  <NConfigProvider :theme="naiveTheme" :theme-overrides="themeOverrides" :hljs="hljsInstance">
    ...
    <template v-else>
      ...
      <ContextEditor
        ...
        :scan-variables="
          (content) => variableManager?.variableManager.value?.scanVariablesInContent(content) || []
        "
        :replace-variables="
          (content, vars) =>
            variableManager?.variableManager.value?.replaceVariables(content, vars) || content
        "
        :isPredefinedVariable="
          (name) => variableManager?.variableManager.value?.isPredefinedVariable(name) || false
        "
...
const parseRouteInfo = () => {
  ...
  return {
    functionMode,
    basicSubMode: (functionMode === 'basic' ? subModeInfo.canonicalSubMode : 'system') as
      | 'system'
      | 'user',
    proSubMode: (functionMode === 'pro' ? subModeInfo.canonicalSubMode : 'variable') as
      | 'multi'
      | 'variable',
    imageSubMode: (functionMode === 'image' ? subModeInfo.canonicalSubMode : 'text2image') as
      | 'text2image'
      | 'image2image',
...
watch(
  ...
  () => {
    ...
    if (
      routeInfo.functionMode === 'basic' &&
      routeInfo.basicSubMode !== globalSettings.state.basicSubMode
    ) {
      globalSettings.updateBasicSubMode(routeInfo.basicSubMode)
    }
    if (
      routeInfo.functionMode === 'pro' &&
      routeInfo.proSubMode !== globalSettings.state.proSubMode
    ) {
      globalSettings.updateProSubMode(routeInfo.proSubMode)
    }
    if (
      routeInfo.functionMode === 'image' &&
      routeInfo.imageSubMode !== globalSettings.state.imageSubMode
    ) {
...
const setWorkspaceRef = (instance: unknown, routeName: WorkspaceRouteName) => {
  ...
  switch (routeName) {
    case 'basic-system':
    case 'basic-user':
      basicModeWorkspaceRef.value = resolvedInstance as typeof basicModeWorkspaceRef.value
      break
    case 'pro-multi':
      systemWorkspaceRef.value = resolvedInstance as typeof systemWorkspaceRef.value
      break
    case 'pro-variable':
      userWorkspaceRef.value = resolvedInstance as typeof userWorkspaceRef.value
      break
  }
}
...
const selectedOptimizationMode = computed<OptimizationMode>(() => {
  ...
  if (routeFunctionMode.value === 'pro')
    return routeProSubMode.value === 'multi' ? 'system' : 'user'
  return 'system'
})
...
const promptPreview = usePromptPreview(promptPreviewContent, promptPreviewVariables, contextMode)
...
const selectedOptimizeModelKey = computed<string>({
  get: () => {
    ...
    if (routeFunctionMode.value === 'pro') {
      const session =
        routeProSubMode.value === 'multi' ? proMultiMessageSession : proVariableSession
      return session.selectedOptimizeModelKey || ''
    }
    if (routeFunctionMode.value === 'image') {
      const session =
        routeImageSubMode.value === 'text2image' ? imageText2ImageSession : imageImage2ImageSession
...
const patchActiveBasicOptimizedResult = (
  ...
  session.updateOptimizedResult({
    optimizedPrompt: partial.optimizedPrompt ?? session.optimizedPrompt ?? '',
...
const basicSessionOptimizedPrompt = computed<string>({
  ...
  set: (value) => patchActiveBasicOptimizedResult({ optimizedPrompt: value || '' }),
})
...
watch(
  [
    isReady,
    () => routeFunctionMode.value,
    () => routeBasicSubMode.value,
    () => routeProSubMode.value,
  ],
  async ([ready]) => {
    // 🔧 只在已完成首次恢复后才响应模式切换
...
watch(currentSelectedTemplate, (newTemplate) => {
  if (sessionManager.isSwitching) return
...
const navigateToSubModeKeyCompat = (toKey: string, opts?: { replace?: boolean }) => {
...
watch(services, async (newServices) => {
  ...
  if (routeFunctionMode.value === 'basic') {
    ...
    await handleContextModeChange(routeProSubMode.value === 'multi' ? 'system' : 'user')
  } else if (routeFunctionMode.value === 'image') {
    ...
    if (newAdvancedMode) {
      if (!optimizationContext.value || optimizationContext.value.length === 0) {
        ...
        if (newOptimizationMode === 'user') {
          optimizationContext.value = [{ role: 'user', content: '{{currentPrompt}}' }]
        }
      }
    }
  },
 ...
 * - 任何需要切换模式/子模式的场景
 */
function navigateToSubModeKey(toKey: SubModeKey, opts?: { replace?: boolean }) {
