<template>
  ...
  >
    ...
    <NSpace vertical :size="16">
      ...
      >
        ...
        <template #header-extra>
          <NButton
            ...
            :title="isPredefinedExpanded ? t('common.collapse') : t('common.expand')"
            :aria-label="isPredefinedExpanded ? t('common.collapse') : t('common.expand')"
      ...
      <NCard size="small">
        ...
        <template #footer>
          <NDivider />
          <div>
            <NText strong depth="1" :style="{ display: 'block', marginBottom: SPACING.MD + 'px' }">
              {{ t('variables.addNew') }}
            </NText>
    ...
    <!-- 导入弹窗 -->
    <VariableImporter v-model:show="showImporter" @import="onVariablesImport" />
      ...
      <NSpace vertical>
        ...
        <NCard size="small" embedded>
          <NSpace justify="space-between" align="center">
            ...
            <NTag type="info"
              >{{ customVariables.length }} {{ t('variables.management.variables') }}</NTag
            >
          </NSpace>
        </NCard>
...
import type { VariableManagerModalProps } from '../../types/components'
...
// 响应式配置
const { modalWidth, buttonSize: responsiveButtonSize, shouldUseCompactMode } = useResponsive()
...
const emit = defineEmits<{
  ...
  (event: 'variableChange', name: string, value: string, action: 'add' | 'update' | 'delete'): void
  ...
  },
  {
    ...
    render: (row: VariableRow) => {
      ...
      return h(
        ...
        { depth: 2 },
        {
          default: () => (description !== descriptionKey ? description : row.name + ' description'),
        }
      )
    },
  },
