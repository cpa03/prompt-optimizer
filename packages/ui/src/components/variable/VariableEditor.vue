<template>
  ...
  >
    <NForm ref="formRef" :model="formData" :rules="formRules" label-placement="top">
      ...
      <NFormItem path="name" :label="t('variables.editor.variableName')" required>
        ...
        <template #feedback>
          <div class="variable-name-feedback">
            <NText depth="3" :style="{ fontSize: FONT_SIZES.XS + 'px' }">
      ...
      <NFormItem path="value" :label="t('variables.editor.variableValue')" required>
        ...
        <div v-if="showPreview" class="variable-preview-section">
          ...
          <div class="preview-content">
            <div class="preview-row">
              ...
              <NButton text size="tiny" class="copy-btn" @click="copyUsage">
                <template #icon>
                  <NIcon><Copy /></NIcon>
                </template>
              </NButton>
    ...
    <template #footer>
      <NSpace justify="end">
        ...
        <NButton type="primary" @click="save" :disabled="!isValid || loading" :loading="loading">
...
const nameInputStatus = computed(() => {
  ...
  return nameValidationState.value.hasError
    ? 'error'
    : nameValidationState.value.isValid
      ? 'success'
      : undefined
})
