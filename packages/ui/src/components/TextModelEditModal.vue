  ...
  >
    <form v-if="formReady" @submit.prevent="handleSubmit">
      <NForm label-placement="left" label-width="auto" size="small">
        ...
        <NH4 style="margin: 0 0 12px 0; font-size: 14px">{{
          t('modelManager.provider.section')
        }}</NH4>
    ...
    <template #action>
      <NSpace justify="space-between" align="center" style="width: 100%">
        <NSpace align="center">
          ...
          <NTag
            v-if="formConnectionStatus"
            :type="formConnectionStatus.type"
            size="small"
            :bordered="false"
          >
  ...
  try {
    ...
    if (typeof errorCode === 'string') {
      try {
        const translated = t(
          errorCode,
          typeof errorParams === 'object' && errorParams
            ? (errorParams as Record<string, unknown>)
            : {}
        )
