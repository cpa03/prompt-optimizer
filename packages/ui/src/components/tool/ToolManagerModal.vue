<template>
  ...
  >
    ...
    <div class="tools-panel">
      <NEmpty v-if="localTools.length === 0" :description="t('contextEditor.noTools')">
        <template #icon>
          <svg
            width="48"
            height="48"
        ...
        <template #extra>
          <NButton @click="addTool" size="medium" type="primary" :disabled="disabled">
      ...
      <NList v-else>
        <NListItem v-for="(tool, index) in localTools" :key="`tool-${index}`">
          <NCard size="small" embedded>
            ...
            <NText depth="3">{{
              tool.function.description || t('contextEditor.noDescription')
            }}</NText>
            <div class="mt-2">
              <NTag size="small">{{
  ...
  <NModal
    ...
    preset="card"
    :title="editingIndex !== null ? t('contextEditor.editTool') : t('contextEditor.addTool')"
    :style="{ width: UI_DIMENSIONS.MODAL_WIDTH_SMALL }"
  >
    <NSpace vertical>
      <!-- 示例提示 -->
      <NAlert v-if="editingIndex === null" type="info" :title="t('contextEditor.exampleTemplate')">
    ...
    <template #action>
      <NSpace>
        ...
        <NButton @click="useWeatherExample" secondary v-if="editingIndex === null">
          {{ t('contextEditor.useExample') }}
        </NButton>
        <NButton @click="useEmptyTemplate" secondary v-if="editingIndex === null">
...
import type { ToolManagerModalProps } from '../../types/components'
...
const saveTool = () => {
  ...
  try {
    ...
    if (editingIndex.value !== null) {
      ...
      emit('toolChange', [...localTools.value], 'update', editingIndex.value)
    } else {
      localTools.value.push(editingTool.value)
      emit('toolChange', [...localTools.value], 'add', localTools.value.length - 1)
    }
    closeToolEditor()
  } catch (e) {
