<template>
  <div class="variable-aware-input-wrapper" :style="completionColorVars">
...
const completionColorVars = computed(() => ({
  '--variable-completion-temporary-color': themeVars.value.successColor || '#18a058',
  '--variable-completion-global-color': themeVars.value.infoColor || '#2080f0',
  '--variable-completion-predefined-color': themeVars.value.warningColor || '#8a63d2',
  '--variable-completion-selected-bg':
    themeVars.value.primaryColorSuppl || 'rgba(32, 128, 240, 0.12)',
  '--variable-completion-selected-color': themeVars.value.primaryColor || '#2080f0',
}))
...
const temporaryVariablesMap = computed(() =>
  buildVariableMap(props.existingTemporaryVariables, props.temporaryVariableValues)
)

const predefinedVariablesMap = computed(() =>
  buildVariableMap(props.predefinedVariables, props.predefinedVariableValues)
)
...
const variableDetectionLabels = computed<VariableDetectionLabels>(() => {
  return {
    ...
    valuePreview: (value: string) => t('variableDetection.valuePreview', { value }),
  }
})
...
const editorHeight = computed(() => {
  ...
  if (typeof autosize === 'boolean') {
    ...
    return autosize ? { min: '100%', max: 'none' } : { min: '200px', max: '200px' }
  }
...
const checkSelection = () => {
  ...
  if (!validation.isValid) {
    ...
    if (validation.reason && validation.reason !== '未选中任何文本') {
  ...
  occurrenceCount.value = Math.max(1, countOccurrencesOutsideVariables(text, selectedText))
  ...
  const startState = EditorState.create({
    ...
    extensions: [
      ...
      missingVariableTooltipCompartment.of(
        missingVariableTooltip(handleAddMissingVariable, variableDetectionLabels.value, {
      ...
      existingVariableTooltipCompartment.of(
        existingVariableTooltip(variableDetectionLabels.value, {
          backgroundColor: themeVars.value.cardColor,
          borderColor: themeVars.value.borderColor,
          borderRadius: themeVars.value.borderRadius,
          textColor: themeVars.value.textColor2,
      ...
      EditorView.updateListener.of((update) => {
        if (update.docChanged) {
          ...
          if (
            ...
            const anchor = Math.min(update.state.selection.main.anchor, trimmed.length)
  ...
  () => {
    ...
    editorView.dispatch({
      effects: [
        ...
        highlighterCompartment.reconfigure(variableHighlighter(extractVariables)),
...
.codemirror-container :deep(.cm-tooltip-autocomplete ul li[aria-selected='true']) {
  background: var(--variable-completion-selected-bg, rgba(32, 128, 240, 0.12));
}

.codemirror-container
  :deep(.cm-tooltip-autocomplete ul li[aria-selected='true'] .cm-completionLabel) {
...
.codemirror-container :deep(.cm-tooltip.cm-completionInfo.cm-completionInfo-right) {
  margin-left: 4px;
}
</style>
