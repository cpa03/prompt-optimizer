...
test.describe('标签管理功能', () => {
  ...
  test('能够打开标签管理器', async ({ page }) => {
    ...
    const tagDialog = page
      .locator('[role="dialog"]')
      .filter({ hasText: /标签管理|Tag Manager/i })
      .last()
...
test.describe('分类管理功能', () => {
  ...
  test('能够打开分类管理器', async ({ page }) => {
    ...
    const dropdownMenu = page
      .locator('.n-dropdown-menu')
      .filter({ hasText: /管理分类|Manage Categories/i })
      .first()
    await expect(dropdownMenu).toBeVisible({ timeout: TIMEOUTS.NAVIGATION.DROPDOWN_VISIBLE })

    await dropdownMenu
      .getByText(/管理分类|Manage Categories/i)
      .first()
      .click()
    ...
    const categoryDialog = page
      .locator('[role="dialog"]')
      .filter({ hasText: /分类管理|Category Manager/i })
      .last()
