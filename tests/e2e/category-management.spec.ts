...
test.describe('分类管理完整流程', () => {
  ...
  async function waitForModalClose(page: any) {
    ...
    const closeButtons = page.locator(
      '[aria-label="close"], .n-base-close, button:has-text("关闭"), button:has-text("关闭")'
    )
    ...
    // 4. 最后等待所有遮罩层消失
    await page
      .waitForSelector('.n-modal-mask', {
        state: 'hidden',
        timeout: TIMEOUTS.NAVIGATION.ELEMENT_HIDDEN,
      })
      .catch(() => {})
  }
  ...
  async function openCategoryManager(page: any) {
    ...
    const managerDialog = page
      .locator('[role="dialog"]')
      .filter({ hasText: /收藏|Favorites/i })
      .first()
  ...
  test('分类创建功能（含颜色选择）', async ({ page }) => {
    ...
    // 查找添加分类按钮
    const addButton = categoryDialog
      .getByRole('button', { name: /添加|新建|创建|add|create/i })
      .first()
    ...
    if ((await nameInput.count()) > 0) {
      ...
      // 填写描述（如果有）
      const descInput = createDialog
        .locator('textarea, input')
        .filter({
          hasText: /描述|description/i,
        })
        .or(createDialog.locator('textarea'))
        .first()
      ...
      if ((await colorPicker.count()) > 0) {
        ...
        if (await colorPanel.isVisible().catch(() => false)) {
          // 点击预设颜色或颜色面板
          const presetColor = colorPanel
            .locator('.n-color-picker-swatch, [class*="swatch"]')
            .first()
  ...
  test('分类排序功能（上移/下移）', async ({ page }) => {
    ...
    if ((await categoryRow.count()) > 0) {
      // 查找上移按钮
      const moveUpButton = categoryRow
        .locator('button')
        .filter({
          hasText: /上移|move up|↑/i,
        })
        .or(categoryRow.locator('button[aria-label*="up"]'))
        .first()
  ...
  test('分类删除保护（有收藏的分类）', async ({ page }) => {
    ...
    const addCategoryButton = categoryDialog.getByRole('button', {
      name: /添加|新建|创建|add|create/i,
    })
    ...
    // 3. 创建一个属于该分类的收藏
    const addFavoriteButton = managerDialog
      .getByRole('button', { name: /添加|创建|新建|add|create/i })
      .first()
