...
test.describe('导入导出完整流程', () => {
      ...
      if ((await confirmButton.count()) > 0) {
        ...
        if ((await importButton2.count()) > 0) {
          ...
          if ((await jsonInput2.count()) > 0) {
            ...
            const confirmButton2 = importDialog2.getByRole('button', {
              name: /确定|确认|导入|ok|import/i,
            })
  ...
  test('导出功能生成有效JSON', async ({ page }) => {
    ...
    // 打开更多菜单导出
    const moreButton = managerDialog
      .getByRole('button')
      .filter({
    ...
    if ((await moreButton.count()) > 0) {
      ...
      if ((await exportOption.count()) > 0) {
        ...
        const downloadPromise = page
          .waitForEvent('download', { timeout: TIMEOUTS.OPERATION.DOWNLOAD })
          .catch(() => null)
  ...
  test('导入后数据可以正常编辑', async ({ page }) => {
    ...
    if ((await jsonInput.count()) > 0) {
      ...
      if ((await confirmButton.count()) > 0) {
        ...
        // 查找并编辑导入的收藏
        const favoriteCard = managerDialog
          .locator('text=可编辑导入收藏')
          .locator('..')
          .locator('..')
        if ((await favoriteCard.count()) > 0) {
          // 查找编辑按钮
          const editButton = favoriteCard
            .locator('button')
            .filter({
              hasText: /编辑|edit/i,
            })
  ...
  test('导入后数据可以正常删除', async ({ page }) => {
    ...
    if ((await jsonInput.count()) > 0) {
      ...
      if ((await confirmButton.count()) > 0) {
        ...
        // 查找并删除导入的收藏
        const favoriteCard = managerDialog
          .locator('text=可删除导入收藏')
          .locator('..')
          .locator('..')
        if ((await favoriteCard.count()) > 0) {
          const deleteButton = favoriteCard
            .locator('button')
            .filter({
              hasText: /删除|delete/i,
            })
          ...
          if ((await deleteButton.count()) > 0) {
            ...
            const confirmDeleteButton = page
              .getByRole('button', { name: /确定|确认|ok|confirm/i })
              .last()
